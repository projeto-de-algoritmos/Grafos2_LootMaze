...
class Player:
    ...
    def draw(self, screen):
        ...
        tile_x = GRID_X_OFFSET + (
                player_x - player_y) * TILE_WIDTH * CONVERSION_FACTOR_Y
        tile_y = GRID_Y_OFFSET + (
                player_x + player_y) * TILE_HEIGHT * CONVERSION_FACTOR_X
        ...
        # Assuming player_sprite is your player's sprite and it's already scaled
        # sprite_width, sprite_height = self.player_image.get_size()

        # Adjust player's position to account for different sprite sizes
        # tile_position = (
        #     700 + player_x * 16 - player_y * 16 - (
        #         CELL_SIZE * 4 - sprite_width
        #     ) / 2,
        #     200 + player_x * 8 + player_y * 8 - (
        #         CELL_SIZE * 5 - sprite_height
        #     )
        # )

        # Adjust player's position to account for depth
        tile_position = (
            tile_position[0], tile_position[1] - CELL_SIZE * 4
        )
    ...
    def load_tile(image_file):
        tile_image = pygame.image.load(
            os.path.join(SPRITES_DIR, image_file)
        ).convert()
